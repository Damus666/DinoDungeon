...
class MapLoader:
        ...
        for pos in self.room_pos:
            ...
            for x in range(pos[0]-1, pos[0]+2):
                for y in range(pos[1]-1, pos[1]+2):
                    adj_pos = (x, y)
                    if adj_pos in room_pos_set and adj_pos != pos:
                        adj_tiles.append(adj_pos)
            for adj_pos in adj_tiles:
                if adj_pos in room_adj:
                    room_adj[adj_pos].update(adj_tiles)
                else:
                    room_adj[adj_pos] = set(adj_tiles)
            if pos in room_adj:
                room_adj[pos].update(adj_tiles)
            else:
                room_adj[pos] = set(adj_tiles)
        self.rooms = []
        visited = set()
        for pos in self.room_pos:
    ...
    def get_connections(self):
        for door, door_extra in self.door_pos:
            ...
            right, left, bottom, top = (
                door[0]+1, door[1]), (door[0]-1, door[1]), (door[0], door[1]+1), (door[0], door[1]-1)
            r_idx, l_idx, b_idx, t_idx = self.get_room_index(right), self.get_room_index(
                left), self.get_room_index(bottom), self.get_room_index(top)

            if r_idx != l_idx:
                data = [l_idx, r_idx, left, right, door,
                        "h", door_extra if door_extra else None]
            elif t_idx != b_idx:
                data = [t_idx, b_idx, top, bottom, door,
                        "v", door_extra if door_extra else None]

            self.connections.append(data)

    @internal
    def get_room_index(self, pos):
        if pos in self.corridor_pos:
            return -1
    ...
    def get_tile_rooms(self, pos):
        dirs = [(pos[0]+1, pos[1]), (pos[0]-1, pos[1]),
    ...
    def get_positions(self, dungeon_map: Map):
        ...
        for tile in dungeon_map.get("objects").tiles:
            data = (self.offset(tile.grid_position),
                    OBJECT_ID_LOOKUP[tile.data.id], tile.extra)
            if data not in self.object_data:
                self.object_data.append(data)

    @internal
    def offset(self, pos): return (
        int(pos[0]-self.player_offset[0]), int(pos[1]-self.player_offset[1]))
